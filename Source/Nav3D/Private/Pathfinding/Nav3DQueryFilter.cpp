...
void UNav3DQueryFilter::InitializeFilter(const ANavigationData& NavData,
                                         const UObject* Querier,
                                         FNavigationQueryFilter& Filter) const
{
	Super::InitializeFilter(NavData, Querier, Filter);
	FNav3DQueryFilter* FilterImpl = static_cast<FNav3DQueryFilter*>(Filter.GetImplementation());
	FilterImpl->QueryFilterSettings = QueryFilterSettings;
}
